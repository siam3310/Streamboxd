...
import { MdArrowDropDown, MdKeyboardArrowUp } from "react-icons/md";
...
export default function Page({ params }: { params: { tv_id: string } }) {
  ...
  return (
    <div className="relative">
            ...
            <div className="relative">
              <Dropdown onClick={() => setShowDropdown(!showDropdown)}>
                <Dropdown.Toggle
                  variant="success"
                  id="dropdown-basic"
                  className="bg-yellow-500 hover:bg-yellow-600 text-black font-semibold px-8 md:px-10 py-3 rounded inline-flex items-center shadow-lg justify-between transition duration-300"
                >
                ...
                <Dropdown.Menu
                  className={`absolute w-[100%] z-0 font-semibold rounded bg-yellow-500 transition duration-300 ${
                    showDropdown
                  ...
                  {Array.from({ length: tvDetails.number_of_seasons }).map(
                    (_, i) => (
                      <Dropdown.Item
                        ...
                        className="text-black py-2 justify-center hover:bg-yellow-600 hover:rounded flex items-center"
                      >
                        {i + 1 === selectedSeason && (
      ...
      <div className="relative z-10 pl-4 pr-2 md:px-64" ref={episodeListRef}>
        {/* Episode List */}
        {loadingEpisodes ? (
          <Loader /> // Show loader while episodes are being fetched
        ) : (
          <h1 className="text-2xl font-bold pt-4  text-white mb-4 ">
